...
int InitElbowStatus = 0;
int StopStatus = 0;
double StopStartTime = 0;
double StopStartM1 = 0;
double StopStartM2 = 0;

int init_flag = 1;
PID_Position wrist_pid_control(30, 1.1, 0.5, 0.01);//腕关节参数设置，前三项为pid参数，最后一项为周期
...
HRESULT CRobotArmMain::CycleUpdate(ITcTask* ipTask, ITcUnknown* ipCaller, ULONG_PTR context)
{
	...
	if (m_PlcToCpp.PlcStarted == true)
	{
		...
		wrist_motion_flag = wrist_motion(50);//wrist_target_angle
...
int CRobotArmMain::wrist_motion(double target)//返回腕关节运动完成标志位
{
	...
	if (wrist_error <= 2 || wrist_fact_angle >= target)//延时检测
	{
		++i;
		if (wrist_error <= 1.5 && i >= 500)//到达预期角度判定
			flag = 1;
		else
			flag = 0;
	}
